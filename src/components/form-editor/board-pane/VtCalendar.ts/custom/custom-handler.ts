...
export class CustomEventHandler {
  ...
  addEvent(event: ICustomEvent) {
    ...
    if (event.type === 'list') {
      const location = this.calendar.getCellLocation(date as any);
      if (!location) {
        return;
      }
      ...
      // bar
      const startLocation = this.calendar.getCellLocation(max([startDate as any, this.calendar.startDate]));
      const endLocation = this.calendar.getCellLocation(min([endDate as any, this.calendar.endDate]));
  ...
  _removeEvent(id: string) {
    ...
    const { date, startDate, endDate } = event;
    if (event.type === 'list') {
      const { col, row } = this.calendar.getCellLocation(date as any);
