  ...
  for (let i = 0; i <= lastKey; i++) {
    ...
    if (!event) {
      ...
      const rect = new Rect({
        ...
        height: barHeight,
        fill: event.bgColor,
        cornerRadius: getRectCornerRadius(startDate as any, endDate as any, cellDate, barCornerRadius),
        pickable: false
      });
      barGroup.add(rect);

      const days = differenceInDays(startOfDay(startDate as any), startOfDay(cellDate));
      const text = new Text({
        x: contentPadding,
        y: contentHeight / 2,
        dx: days * width,
