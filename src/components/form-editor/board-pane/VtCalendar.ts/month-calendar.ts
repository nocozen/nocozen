...
import { defaultDayTitles, getWeekdays, getRecords, getStartAndEndDate } from './date-util';
...
export class Calendar extends EventTarget {
  ...
  constructor(container: HTMLElement, options?: CalendarConstructorOptions) {
    ...
    this.showToday = this.options.showTitle ?? true;   // todo: showTitle showToday 参数设置待完善
  ...
  _createTable() {
    ...
    const week = (dayTitles ?? getWeekdays()) as DateRecordKeys[];
    ...
    const option = createTableOption(
      ...
      this.showToday ? this.currentDate : undefined,
      {
        ...
        containerHeight: this.container.clientHeight + 20     // todo: 临时修正不显示标题后底边高度留白问题，正确修改位置待确认
      }
    );
  ...
  _updateMonthCustomStyle(year: number, month: number) {
    ...
    this.currentMonthCellRanges?.forEach(range => {
      this.table!.arrangeCustomCellStyle(range, null as any);
    });
