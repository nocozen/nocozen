    ...
    <NFlex vertical :size="0" class="h-full w-full min-w-230 ">
      ...
      <NFlex class="w-full flex-nowrap bg-gray-100/50" :size="0" style="height: calc(100% - 50px);">
        ...
        <NFlex class="h-full p-4 outline outline-slate-200/50 outline-1" style="width: calc(100% - 560px);">
          <DropBox boxType="rowDims" :i="chartId" :list="cptRowDims" @updateList="updateRowDims"
            :acceptsType="[BiType.Text, BiType.Date, BiType.Number]" boxName="行维度"  labelWidth="44px"/>
          <DropBox boxType="colDims" :i="chartId" :list="cptColDims" @updateList="updateColDims"
            :acceptsType="[BiType.Text, BiType.Date, BiType.Number]" boxName="列维度"  labelWidth="44px"/>
          <DropBox boxType="pmeas" :i="chartId" :list="cptMetrics" @updateList="updateMetrics"
            :acceptsType="[BiType.Text, BiType.Date, BiType.Number]" boxName="指标"  labelWidth="44px"/>
          <DropBox boxType="filter" :i="chartId" :list="cptFilter" @updateList="updateFilter"
            :acceptsType="[BiType.Text, BiType.Date, BiType.Number]" boxName="过滤"  labelWidth="44px"/>
          <NFlex style="width: 100%;height: calc(100% - 168px);" :size="0" class="bg-white">
            <QbTable ref="refTable" v-if="compConfig" :i="compConfig.i" type="pivot" @updateTitle="updateTitle" />
          </NFlex>
        </NFlex>
...
const updateDataBind = (dataBind: Meta.DataBind) => {
  if (compConfig.value && updateCompConfig) {
    ...
    compConfig.value.rowDims && (compConfig.value.rowDims.length = 0);
    compConfig.value.colDims && (compConfig.value.colDims.length = 0);
...
const cptRowDims = computed(() => {
  return compConfig.value?.rowDims;
})

const cptColDims = computed(() => {
  return compConfig.value?.colDims;
})
...
const updateRowDims = (list: any) => {
  compConfig.value
    && (compConfig.value.rowDims = list)
    && (updateCompConfig && updateCompConfig(compConfig.value as Meta.CompConfig));
  refresh();
}
const updateColDims = (list: any) => {
  compConfig.value
    && (compConfig.value.colDims = list)
    && (updateCompConfig && updateCompConfig(compConfig.value as Meta.CompConfig));
  refresh();
}
