        ...
        <NFlex class="h-full p-4 outline outline-slate-200/50 outline-1" style="width: calc(100% - 560px);">
          <NFlex :size="0" class="w-full">
            <DropBox boxType="dims" :i="chartId" :list="cptDims" @updateList="updateDims"
              :acceptsType="[BiType.Text, BiType.Date, BiType.Number]" boxName="维度"  class="flex-1" labelWidth="56px" />
            <DropBox boxType="taskTitle" :i="chartId" :list="cptTaskTitle" @updateList="updateTaskTitle"
              :acceptsType="cptAcceptTagType" boxName="数据标签"  class="max-w-54" labelWidth="56px" />
          </NFlex>
          <NFlex :size="0" class="w-full">
            ...
            <DropBox boxType="endDate" :i="chartId" :list="cptEndDate" @updateList="updateEndDate"
              ...
              labelWidth="56px" />
            <DropBox boxType="progress" :i="chartId" :list="cptProgress" @updateList="updateProgress"
              :acceptsType="cptAcceptProgressType" boxName="进度" class="max-w-54" labelWidth="56px" />
          </NFlex>
          ...
          <NFlex style="width: 100%;height: calc(100% - 132px);" :size="0" class="bg-white">
            <QbGantt ref="refGantt" v-if="compConfig" :i="compConfig.i" type="pivot" @updateTitle="updateTitle" />
          </NFlex>
        </NFlex>
...
const cptAcceptTagType = computed(() => {
  if (!compConfig.value?.taskTitle || compConfig.value?.taskTitle?.length == 0) {
    return [BiType.Text];
  } else {
    return []
  }
...
const cptAcceptProgressType = computed(() => {
  if (!compConfig.value?.progress || compConfig.value?.progress?.length == 0) {
...
const refresh = () => {
  refGantt.value && refGantt.value.reLoadGantt();
}
...
const updateDataBind = (dataBind: Meta.DataBind) => {
  if (compConfig.value && updateCompConfig) {
    ...
    compConfig.value.taskTitle && (compConfig.value.taskTitle.length = 0);
...
const cptTaskTitle = computed(() => {
  return compConfig.value?.taskTitle;
})
...
const cptProgress = computed(() => {
  return compConfig.value?.progress;
})
...
const updateTaskTitle = (list: any) => {
  compConfig.value
    && (compConfig.value.taskTitle = list)
    && (updateCompConfig && updateCompConfig(compConfig.value as Meta.CompConfig));
    refresh();
}
// 任务开始时间
...
const updateProgress = (list: any) => {
  compConfig.value
    && (compConfig.value.progress = list)
    && (updateCompConfig && updateCompConfig(compConfig.value as Meta.CompConfig));
    refresh();
}
