...
import { ref, inject,provide,Ref, computed, watch, onBeforeUnmount, onMounted, useSlots } from "vue"
...
import useCurrentInstance from "@/hooks/common/useInstance"
...
// 2025.2.27 qyk: 解决GridItem中 onMounted 方法中 thisLayout?.colNum 获取不到最新值问题。
const newColNum = inject<Ref<number>>('newColNum');
...
const margin = ref<Array<number>>([0, 0])
...
// provide<Ref<boolean | null>>('gridItemDraggable', draggable);    // qyk 2024.11.27: 解决表格编辑拖拽冲突
...
onMounted(() => {
  ...
  // }
  // qyk: 替代上述代码，解决 thisLayout?.colNum 获取不到最新值问题。2025.2.27
  newColNum && (cols.value = newColNum.value)

  rowHeight.value = thisLayout?.rowHeight as number
  containerWidth.value = thisLayout?.width !== null ? (thisLayout?.width as number) : 100
  margin.value = thisLayout?.margin !== undefined ? thisLayout.margin : [0, 0]
...
function calcXY(top: number, left: number) {
  ...
  // console.log(cols.value,innerW.value)
  // Capping
...
// todo: vue-grid-placeholde中使用阴影扩展范围实现内边距只能使用一个参数，使用两个参数需要再完善；
// const itemPadding = computed(() => (`${props.padding[0]}px ${props.padding[1]}px`))
let itemPadding = computed(() => (`${props.padding[0]}px`));
let resizablePB = computed(() => (`${props.padding[0]}px`));
let resizablePR = computed(() => (`${props.padding[1]}px`));
...
$itemPadding: v-bind(itemPadding);

.vue-grid-item {
  padding: $itemPadding;
  touch-action: none;
  /* qyk 2024.8.23 增加 解决问题：Consider adding CSS "touch-action: none" to this element todo: 组件的no-touch样式的作用*/
...
.vue-grid-item.vue-grid-placeholder {
  box-shadow: inset 0 0 0 $itemPadding white;
  background: red;
  opacity: 0.2;
...
.vue-grid-item>.vue-resizable-handle {
  ...
  width: 12px;
  height: 12px;
  bottom: v-bind(resizablePB);
  right: v-bind(resizablePR);
  background: url("data:image/svg+xml;base64,");
