/**
 * fork: performant-array-to-tree v1.11.0
 * 
 */
...
// label?: string 未启用
// function addNodeParents(parents: Array<any>, treeNode: Array<any>, label?: string) {
  function addNodePaths(parents: Array<any>, treeNode: Array<any>) {
    for (let menu of treeNode) {
      menu["parentIds"] = [...parents, menu.key];
      menu.children && addNodePaths(menu["parentIds"], menu.children);
    }
  }
  
  // children空数组值修改为：undefined
  export function cleanEmptyChildren(tree: any[]) {
  ...
  /**
   * 固定属性名称：data: {id: "", parentId: "", label: ""}, children: [{}], paths: [{}]
   * @param items 
   * @returns 
   */
  export function arrToMenu( items: Item[]): TreeItem[]{
    const tree = arrayToTree( items, { id: '_id', dataField: null } );
    addNodePaths([], tree)
    return tree;
  }